...
source "amazon-ebs" "amazon-linux-2" {
  ami_name      = "Andrii-Klimakhin-awslinux-2-${local.timestamp}"
...
build {
  ...
  provisioner "shell" {
    inline = [
      ...
      "docker build . -t nginxhw15 -f /tmp/Dockerfile",
      "docker run -d -p ${local.port}:80 --restart always nginxhw15"
    ]
  }
